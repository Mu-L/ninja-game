...
var allies_data: Array[AllyData]
...
static func create(allies_data: Array[AllyData], battle_data: BattleData) -> Battle:
	...
	if text_box.visible and event.is_action_pressed("primary action"):
		...
		if event.is_action_pressed("primary action"):
			...
			if ally.played_turn or not ally.is_alive:
				...
				return
			state = States.ALLY_PLAYING_TURN
...
func end_ally_turn() -> void:
	...
	await get_tree().create_timer(0.1).timeout
	state = States.ENEMY_PLAYING_TURN
	enemy_turn()
...
func set_state(new_state: States) -> void:
	...
	match new_state:
		...
		States.CHOOSING_TO_END_TURN:
			...
			confirm_label.show()
		States.ALLY_PLAYING_TURN:
			cancel_label.show()
		States.ENEMY_PLAYING_TURN:
