...
func _ready() -> void:
	...
	EventBus.set_cursor_visible.connect(func(val: bool):
		self.visible = val
	)
...
func move_to(pos: Vector2) -> void:
	...
	tween.tween_property(EventBus, "is_cursor_moving", true, 0.01)
	tween.tween_property(self, "global_position", pos, 0.15)
	tween.tween_property(EventBus, "is_cursor_moving", false, 0.01)
